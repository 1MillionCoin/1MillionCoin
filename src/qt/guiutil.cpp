...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("1MillionCoin"))
        return false;
...
{
    // Convert 1MillionCoin:// to 1MillionCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("1MillionCoin://"))
    {
        uri.replace(0, 12, "1MillionCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "1MillionCoin.lnk";
}
...
{
    return GetAutostartDir() / "1MillionCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("1MillionCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  1MillionCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("1MillionCoin-Qt"));
