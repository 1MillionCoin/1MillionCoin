...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "1MillionCoin " + FormatFullVersion();
...
void ThreadOpenAddedConnections()
{
    ...
    if (HaveNameProxy()) {
        while(true) {
            ...
            MilliSleep(130000); // Retry every 2 minutes
        }
    }
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. 1MillionCoin is probably already running."), addrBind.ToString());
        else
