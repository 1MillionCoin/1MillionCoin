...
int nAnonymize1MillionCoinAmount = 500;
...
#else
    const char* pszModule = "1MillionCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\1MC
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\1MC
    // Mac: ~/Library/Application Support/1MC
    // Unix: ~/.1MC
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "1MC";
#else
    fs::path pathRet;
...
void createConf()       //Automatic 1MillionCoin.conf generation
{
    srand(time(NULL));
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "1MillionCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "1millioncoind.pid"));
