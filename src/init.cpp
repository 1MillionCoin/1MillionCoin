...
void Shutdown()
{
    ...
    RenameThread("1millioncoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: 1MillionCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: 1millioncoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 11111 or testnet: 11112)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 11111 or testnet: 11113)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymize1MillionCoinamount=<n> " + _("Keep N 1MillionCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. 1MillionCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. 1MillionCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("1MillionCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "1MillionCoin server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of 1MillionCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart 1MillionCoin to complete") << "\n";
    ...
    nAnonymize1MillionCoinAmount = GetArg("-anonymize1MillionCoinamount", 0);
    if(nAnonymize1MillionCoinAmount > 999999) nAnonymize1MillionCoinAmount = 999999;
    if(nAnonymize1MillionCoinAmount < 2) nAnonymize1MillionCoinAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymize1MillionCoinAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize 1MillionCoin Amount %d\n", nAnonymize1MillionCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       11MillionCoin+1000 == (.11MillionCoin+100)*10
       101MillionCoin+10000 == (11MillionCoin+1000)*10
    */
